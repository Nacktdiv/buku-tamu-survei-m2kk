import { HiBookOpen, HiDocumentReport } from "react-icons/hi";
...
function Hero ({ setIsGuestBookOpen, setIsSurveyOpen }) {
    return(
        ...
        <div className="relative bg-primary h-[45vh] rounded-b-[30%] flex flex-col items-center justify-center">
            ...
            <p className="md:text-2xl max-w-[70%] text-center text-white mt-4 md:mt-0 md:-translate-y-full">Catat dan Dokumentasikan Kunjungan Anda</p>
        </div>
        <div className="flex flex-col md:flex-row justify-center items-center gap-10 mt-5 md:mt-0">
            <Card bg='bg-accent-green' bgbtn='bg-white' tittle='Buku Tamu' desc='Silakan isi buku tamu untuk mencatat kunjungan Anda.' textbtn='Isi Buku Tamu' icon={<HiBookOpen  />} onClick={() => setIsGuestBookOpen(true)}></Card>
            <Card bg='bg-white' bgbtn='bg-accent-green' tittle='Survey' desc='Berikan nilai dan masukan untuk perbaikan layanan.' textbtn='Mulai Survei' icon={<HiDocumentReport  />} onClick={() => setIsSurveyOpen(true)}></Card>
        </div>
        </>
    )
}
    ...
    return (
        <div className={`flex w-60 h-30 md:w-100 md:h-50 lg:w-120 lg:h-60 md:-translate-y-1/2 ${bg} border-4 border-accent-green rounded-xl shadow-2xl`}>
            ...
            <div className=" flex flex-col gap-2 lg:gap-4 py-4 px-2 md:py-8 md:px-6 justify-center w-full">
                    ...
                    <button className={` text-xs md:text-sm lg:text-base font-bold text-primary border-solid border-2 border-primary-light ${bgbtn} rounded-full h-full `} onClick={onClick}>{textbtn}</button>
            </div>
        </div>
    )
}
